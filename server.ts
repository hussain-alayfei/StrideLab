...
app.post("/api/ai/extend-plan", async (req, res) => {
  try {
    ...
    const ai = getAI();
    const prompt = `أنت مساعد تدريبي ذكي، ومهمتك ليست وضع خطة من عندك بل **تعلّم أسلوب المدرب المحدد** من خططه السابقة ثم إكمال الشهر التالي بنفس بصمته التدريبية.

الخطة/الخطط السابقة التي صممها هذا المدرب بنفسه (استخدمها كمرجع لأسلوبه):
${JSON.stringify(currentWorkouts)}

حلّل جيداً من الخطط أعلاه العناصر التالية التي تميّز أسلوب هذا المدرب تحديداً:
1. طريقة تسمية التمارين ونبرتها اللغوية (هل يستخدم مصطلحات إنجليزية بين قوسين؟ هل الأسماء مختصرة أم وصفية؟).
2. توزيع أنواع التمارين على الأسبوع (كم تمرين سهل/تمبو/فترات/طويل/استشفاء، وترتيب الأيام).
3. أسلوب كتابة الوصف الفني (مستوى التفصيل، ذكر نطاقات النبض، التقسيم لمراحل).
4. نسبة الشدة والمسافات المعتادة لديه.

المطلوب: صمم جدول الشهر رقم ${currentMonth + 1} بحيث يبدو **وكأن نفس المدرب كتبه بيده** — نفس أسلوب التسمية، نفس التوزيع، نفس نبرة الأوصاف — مع تطوير طبيعي للأحمال.

قيود السلامة الإلزامية (Progressive Overload):
- لا تتعدَّ الزيادة في المسافات الأسبوعية أو التمارين الفردية حاجز 10٪ مقارنةً بالشهر السابق.
- حافظ على نفس عدد أيام الراحة/الاستشفاء التي اعتمدها المدرب.
هدف العداء الإجمالي: ${athleteGoal}

أخرج النتيجة في مصفوفة JSON مطابقة للهيكل المطلوب، مع محاكاة أسلوب المدرب لا استبداله.`;

    const response = await ai.models.generateContent({
      model: MODEL,
      contents: prompt,
      config: {
        systemInstruction: "أنت مساعد يتعلّم أسلوب المدرب من خططه السابقة ويكمل الأشهر التالية بنفس بصمته (التسمية، التوزيع، نبرة الأوصاف) بدلاً من فرض أسلوبك الخاص. أخرج JSON متكاملاً يحافظ على هيكلية التمارين ويحاكي نمط المدرب.",
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          properties: {
...
// 8. Nutrition Plan Generation (خطة تغذية مرافقة للخطة التدريبية — إضافة مميّزة)
const NUTRITION_FALLBACK = [
  { name: "وجبة ما قبل التدريب", description: "شوفان بالموز والعسل مع قهوة خفيفة قبل الركض بساعة لطاقة هوائية مستقرة.", calories: "~350 سعرة" },
  { name: "وجبة التعافي بعد الجري", description: "بروتين (صدر دجاج أو بيض) مع أرز وخضار لتعويض الجلايكوجين وترميم العضلات خلال 45 دقيقة.", calories: "~520 سعرة" },
  { name: "وجبة خفيفة مسائية", description: "زبادي يوناني مع مكسرات وتمر لدعم الاستشفاء ونوم أعمق.", calories: "~240 سعرة" }
];

app.post("/api/ai/generate-nutrition", async (req, res) => {
  try {
    const { athleteGoal, weeklyKm, weight } = req.body;

    if (!process.env.GEMINI_API_KEY) {
      return res.json({ meals: NUTRITION_FALLBACK });
    }

    const ai = getAI();
    const response = await ai.models.generateContent({
      model: MODEL,
      contents: `صمّم خطة تغذية يومية مبسّطة لعدّاء بالمواصفات التالية:
- الهدف: ${athleteGoal}
- الحِمل الأسبوعي: ${weeklyKm} كم
- الوزن: ${weight || 'غير محدد'} كجم

المطلوب: 3 إلى 4 وجبات عملية وواقعية (ما قبل التدريب، التعافي بعده، ووجبات داعمة) بأطعمة متوفرة محلياً، مع سعرات تقريبية، بلغة عربية رياضية واضحة.`,
      config: {
        systemInstruction: "أنت أخصائي تغذية رياضية للعدّائين. صغ الخطة بدقة في JSON بلغة عربية عملية وموجزة.",
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            meals: {
              ...
              items: {
                type: Type.OBJECT,
                properties: {
                  name: { type: Type.STRING, description: "اسم الوجبة أو توقيتها" },
                  description: { type: Type.STRING, description: "مكونات الوجبة والغرض منها" },
                  calories: { type: Type.STRING, description: "السعرات التقريبية، مثل '~350 سعرة'" }
                },
    ...
    const result = JSON.parse(response.text || '{"meals":[]}');
    res.json({ meals: result.meals && result.meals.length > 0 ? result.meals : NUTRITION_FALLBACK });
  } catch (error: any) {
    console.warn("Warning in generate-nutrition:", error?.message || error);
    res.json({ meals: NUTRITION_FALLBACK });
  }
});
